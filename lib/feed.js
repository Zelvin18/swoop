...
export async function createPost({
  ...
  filterName = 'Original', musicTrackId = null, musicStartSec = 0,
  // service fields
  ...
  if (mediaFiles.length) {
    ...
    if (photos.length) { const u = await uploadPostMedia(photos, sellerId); imageUrls.push(...u) }
    if (videos.length) { const u = await uploadPostMedia(videos, sellerId); videoUrl = u[0] || null }
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      title, description, caption,
      price:      postType !== 'social' ? (parseFloat(price) || null) : null,
