    ...
    .select(`
      *,
      seller:profiles!seller_id (
        id, full_name, username, avatar_url, verified, location, rating
      )
    `)
...
export async function makeOffer({ requestId, sellerId, message, price }) {
  const { data, error } = await supabase
    .from('offers')
    .insert({ request_id: requestId, seller_id: sellerId, message, price: price ? parseFloat(price) : null })
    .select()
    .single()
  if (error) { console.error('makeOffer', error); return null }
